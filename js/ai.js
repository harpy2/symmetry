...
async function generateCombatAI(enemy, enemyCount, isBoss) {
  ...
  const ctx = {
    ...
    skills: availSkills.map(s => `${s.icon}${s.name}: ${s.desc||'기본 공격'}`),
    passives: (G.equippedPassives||[]).map(p => `${p.icon}${p.name}: ${p.desc||''}`),
    party: (G.party||[]).filter(p=>p&&p.className).map(p=>({
      class:p.className,level:p.level||1,
      skills:(p.equippedSkills||[]).map(s=>`${s.icon}${s.name}: ${s.desc||''}`),
      passives:(p.equippedPassives||[]).map(p2=>`${p2.icon}${p2.name}: ${p2.desc||''}`)
    })),
...
function generateCombatLocal(enemy, enemyCount, isBoss) {
  ...
  const singleHP = Math.floor((isBoss ? (60 + G.floor * 8) : (20 + G.floor * 3)) * floorScale);
  let enemies = [];
    ...
    for (const member of partyMembers) {
      ...
      for (const attacker of actual) {
        ...
        if (eRoll < evadeChance) {
          ...
          const rawDmg = (isBoss ? (10 + G.floor * 2) : (8 + G.floor * 1)) * floorScaleE * lvlScaleP * (eCrit ? 2.0 : (0.7 + Math.random() * 0.5));
      ...
      if (stillAlive.length > 0 && !enemyStunned && !member._invincible) {
        ...
        for (const attacker of actualAttackers) {
          ...
          if (eRoll < evadeChance) {
            ...
            const rawDmg = (isBoss ? (10 + G.floor * 2) : (8 + G.floor * 1)) * floorScale * lvlScale * (eCrit ? 2.0 : (0.7 + Math.random() * 0.5)) * fearMult;
